...
Let's test this in copilot. Can I create a chunk that calculates the mean of a variable? What about windsorizing the mean?

# Make a code chunk that calculates the mean of a variable

```{r mean}
# Calculate the meaan of a variable
...
binscatter <- function(nbins) { #note default choice
  ...
  mydata %>% group_by(bin) %>%
    summarise(xmean = mean(Age,na.rm=T), ymean = mean(BMI,na.rm=T)) %>%
...
binscatter(1) # what happens if I play around? Can you add other arguments?
...
cov(mydata$BMI, mydata$Age,use="pairwise.complete.obs") # Base covariance -- but 65 what?
cov(mydata$BMI*100, mydata$Age,use="pairwise.complete.obs")

cor(mydata$BMI, mydata$Age,use="pairwise.complete.obs") # This makes more sense!
